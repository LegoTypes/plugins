...
class M2_0_0 extends BaseModelMigration
{
    public function run($model)
    {
        parent::run($model);
        try {
            require_once '/usr/local/opnsense/scripts/OPNsense/WGClientTunnels/lib/tunnels.php';

            $rows = [];
            $old = Config::getInstance()->object()->OPNsense->WGClientTunnels ?? null;
            ...
            $heldNames = [];
            $raw = @file_get_contents(WGCT_LEGACY_HELD_FILE);
            $decoded = $raw !== false ? json_decode($raw, true) : null;
            if (is_array($decoded)) {
                $heldNames = array_values(array_filter($decoded, 'is_string'));
            }

            $plan = wgct_migration_plan(wgct_core_snapshot(), $rows, $heldNames);
            ...
            throw new \Exception('WGClientTunnels 2.0.0 migration: ' . $e->getMessage(), 0, $e);
        }
    }
}
