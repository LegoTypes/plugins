 ...
 * gateways. wgct_refs_from_tree() is pure over a SimpleXML tree, so the
 * self-test runs it on a synthetic config; wgct_refs_snapshot() applies it
 * to the live one.
 */
 ...
 */
function wgct_member_tokens(string $members): array {
    $out = [];
 ...
 */
function wgct_refs_from_tree(\SimpleXMLElement $root, array $groups): array {
    $refs = [];
    ...
    $csv = fn (\SimpleXMLElement $n, string $field): array => wgct_split_csv((string)$n->{$field});

    foreach ($groups as $name => $members) {
        $add('group:' . $name, 'gateway group ' . $name, $members, []);
    }
    foreach (wgct_xml_list($root, 'interfaces') as $interfaces) {
    ...
    foreach (wgct_xml_list($root, 'filter', 'rule') as $i => $r) {
    ...
    foreach (wgct_xml_list($root, 'nat', 'rule') as $i => $r) {
    ...
    foreach (wgct_xml_list($root, 'nat', 'outbound', 'rule') as $i => $r) {
        $add($id('outbound', $r, $i), $label('legacy outbound NAT rule', $r, 'descr', $i), [],
            array_merge($csv($r, 'interface'), [(string)$r->source->network, (string)$r->destination->network]));
    }
    foreach (wgct_xml_list($root, 'OPNsense', 'Firewall', 'Filter', 'rules', 'rule') as $i => $r) {
    ...
    foreach (wgct_xml_list($root, 'OPNsense', 'Firewall', 'Filter', 'snatrules', 'rule') as $i => $r) {
    ...
    foreach (wgct_xml_list($root, 'OPNsense', 'Firewall', 'Filter', 'npt', 'rule') as $i => $r) {
        $add($id('npt', $r, $i), $label('NPTv6 rule', $r, 'description', $i), [],
            array_merge($csv($r, 'interface'), $csv($r, 'trackif')));
    }
    foreach (wgct_xml_list($root, 'OPNsense', 'Firewall', 'Filter', 'onetoone', 'rule') as $i => $r) {
        $add($id('onetoone', $r, $i), $label('1:1 NAT rule', $r, 'description', $i), [],
            array_merge($csv($r, 'interface'), $csv($r, 'source_net'), $csv($r, 'destination_net')));
    }
    foreach (wgct_xml_list($root, 'staticroutes', 'route') as $i => $r) {
        $add($id('route', $r, $i), 'static route ' . (string)$r->network, [(string)$r->gateway], []);
    }
    foreach (wgct_xml_list($root, 'ifgroups', 'ifgroupentry') as $g) {
        $add('ifgroup:' . (string)$g->ifname, 'interface group ' . (string)$g->ifname, [], wgct_member_tokens((string)$g->members));
    }
    foreach (wgct_xml_list($root, 'bridges', 'bridged') as $i => $b) {
        $add($id('bridge', $b, $i), $label('bridge', $b, 'bridgeif', $i), [], wgct_member_tokens((string)$b->members));
    }
    foreach (wgct_xml_list($root, 'gres', 'gre') as $i => $g) {
        $add($id('gre', $g, $i), $label('GRE tunnel', $g, 'greif', $i), [], wgct_member_tokens((string)$g->if));
    }
    foreach (wgct_xml_list($root, 'gifs', 'gif') as $i => $g) {
        $add($id('gif', $g, $i), $label('GIF tunnel', $g, 'gifif', $i), [], wgct_member_tokens((string)$g->if));
    }
    foreach (wgct_xml_list($root, 'virtualip', 'vip') as $i => $v) {
 ...
 *
 * @param array<string, list<string>> $groups wgct_core_snapshot()['groups']
 * @return list<array{id: string, what: string, gateways: list<string>, interfaces: list<string>}>
 */
function wgct_refs_snapshot(array $groups): array {
    return wgct_refs_from_tree(\OPNsense\Core\Config::getInstance()->object(), $groups);
}
 ...
 * opt13.
 *
 * @param array        $refs         wgct_refs_from_tree()
 ...
 */
function wgct_remove_refusals(array $refs, array $gatewayNames, string $opt, array $ownIds): array {
 ...
 */
function wgct_refs_selftest(): int {
    $t = ['fail' => 0, 'total' => 0];
    ...
    $cases = [
        ...
        ['an interface group member with a _vip suffix', [$ref('ifgroup:x', [], wgct_member_tokens('opt3 opt13_vip2'))], 1],
        ['a bridge member', [$ref('bridge:0', [], wgct_member_tokens('opt13,opt3'))], 1],
    ...
    foreach ($cases as [$desc, $refs, $expect]) {
        wgct_check($t, "refs: {$desc} => " . ($expect === 1 ? 'refused' : 'allowed'),
            count(wgct_remove_refusals($refs, $gws, 'opt13', $own)) === $expect);
    }
    wgct_check($t, 'refs: a refusal names the object and what it uses',
        wgct_remove_refusals([$ref('rule:7', ['tun_c'], ['opt13'])], $gws, 'opt13', []) === ['rule:7 uses gateway tun_c and interface opt13']);
    wgct_check($t, 'refs: an unassigned tunnel (no interface) matches no empty token',
        wgct_remove_refusals([$ref('rule:8', [], [''])], $gws, '', []) === []);
    wgct_check($t, 'refs: member tokens split on commas and spaces, _vip stripped',
        wgct_member_tokens('opt3, opt13_vip2  lan') === ['opt3', 'opt13', 'lan']);

    $x = simplexml_load_string('<opnsense><filter><rule><a>1</a></rule><rule/></filter></opnsense>');
    wgct_check($t, 'xml_list: every element on the path, none for a missing path',
        $x !== false && count(wgct_xml_list($x, 'filter', 'rule')) === 2 && wgct_xml_list($x, 'nat', 'outbound', 'rule') === []);
...
XML;
    ...
    $refs = $root !== false ? wgct_refs_from_tree($root, ['grp' => ['tun_c', 'WAN_A']]) : [];
    $out = wgct_remove_refusals($refs, $gws, 'opt13', ['snat:s-1', 'if:opt13']);
    ...
    wgct_check($t, 'refs: every section of a config tree is found, legacy spellings included (16 refusals)',
        count($out) === 16
    ...
    wgct_check($t, 'refs: the same tree does not refuse an unrelated tunnel',
        wgct_remove_refusals($refs, ['tun_x', 'tun_x-ipv6'], 'opt14', []) === []);

    return wgct_tally_report('refs', $t);
}
