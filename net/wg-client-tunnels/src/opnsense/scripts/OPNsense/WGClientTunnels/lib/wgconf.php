 ...
 * wgct_derive_pubkey() is pure. The private and preshared keys travel only
...
const WGCT_NAME_PATTERN = '/^[a-zA-Z0-9_-]{1,27}$/';
const WGCT_TUNNEL_MTU_MIN = 1280;
 ...
 */
function wgct_is_wg_key(#[\SensitiveParameter] string $key): bool {
    if (strlen($key) !== 44) {
        return false;
    }
 ...
 */
function wgct_normalize_address(string $address): ?array {
        ...
        if ($section === '[interface]' && $interfaces === 1) {
            if ($key === 'privatekey') {
                ...
                foreach (explode(',', $value) as $address) {
                    ...
                    $norm = wgct_normalize_address($address);
                    if ($norm === null) {
 ...
 */
function wgct_request_flag(mixed $v): array {
    if ($v === null) {
        return [true, null];
    }
    if ($v === true || $v === '1' || $v === 1) {
 ...
 *
 * @param array $raw         keys of WGCT_CREATE_FIELDS. ipv6/unique: see wgct_request_flag()
...
function wgct_create_request(#[\SensitiveParameter] array $raw, bool $mtuRequired): array {
    ...
    $name = $str('name');
    if (preg_match(WGCT_NAME_PATTERN, $name) !== 1) {
 ...
 *
 * @param string $privkey a key wgct_is_wg_key() accepted
 * @return string|null the public key, or null when wg rejected the key
 */
function wgct_derive_pubkey(#[\SensitiveParameter] string $privkey): ?string {
    $proc = proc_open(
        ['/usr/bin/wg', 'pubkey'],
    ...
    fclose($pipes[1]);
    return proc_close($proc) === 0 && wgct_is_wg_key($out) ? $out : null;
}
 ...
 * @return string
 */
function wgct_redact(string $message, #[\SensitiveParameter] array $secrets): string {
 ...
 */
function wgct_wgconf_selftest(): int {
    $t = ['fail' => 0, 'total' => 0];
    ...
    wgct_check($t, 'wgconf: provider config => both addresses, peer key, endpoint; DNS ignored',
        $p['errors'] === []
    ...
    wgct_check($t, 'wgconf: no IPv6 address => inet6 empty, no error',
        $p['errors'] === [] && $p['public']['addresses']['inet6'] === []);

    $p = wgct_parse_wgquick(str_replace("Address = 10.2.0.2/32, 2001:db8::2:2/128\n", "Address = 10.2.0.2\nAddress = 2001:db8::2:2\n", $base));
    wgct_check($t, 'wgconf: several Address lines without prefixes => collected, /32 and /128 added',
        $p['errors'] === [] && $p['public']['addresses'] === ['inet' => ['10.2.0.2/32'], 'inet6' => ['2001:db8::2:2/128']]);

    $p = wgct_parse_wgquick(str_replace('Endpoint', "PresharedKey = {$psk}\nEndpoint", $base));
    wgct_check($t, 'wgconf: PresharedKey => kept as a secret, has_psk',
    ...
    $p = wgct_parse_wgquick(str_replace("PrivateKey = {$priv}\n", '', $base));
    wgct_check($t, 'wgconf: no PrivateKey => refused', $p['errors'] === ['PrivateKey is missing']);

    $p = wgct_parse_wgquick(str_replace('10.2.0.2/32, 2001:db8::2:2/128', '10.2.0.2/32, 10.2.0.3/32', $base));
    wgct_check($t, 'wgconf: two IPv4 addresses => refused',
        $p['errors'] === ['Address has several IPv4 addresses; exactly one is supported']);

    $p = wgct_parse_wgquick(str_replace("\n", "\r\n", $base));
    wgct_check($t, 'wgconf: CRLF line ends => same result', $p['errors'] === [] && $p['public']['endpoint_ip'] === '198.51.100.10');

    $p = wgct_parse_wgquick(str_replace("PrivateKey = {$priv}", 'PrivateKey = ' . substr($priv, 0, 43), $base));
    wgct_check($t, 'wgconf: a key one character short => refused, and the message does not quote it',
    ...
    $r = wgct_create_request($json, false);
    ...
    $r = wgct_create_request($json + ['ipv6' => true, 'unique' => 0], false);
    wgct_check($t, 'request: JSON bool and int flags are read as given',
        $r['errors'] === [] && $r['req']['ipv6'] === true && $r['req']['unique'] === false);
    $r = wgct_create_request($json + ['ipv6' => null, 'unique' => null], false);
    wgct_check($t, 'request: explicit JSON null flags mean the default, like absent ones',
        $r['errors'] === [] && $r['req']['ipv6'] === null && $r['req']['unique'] === null);
    foreach ([['"false"', 'false'], ['"true"', 'true'], ['""', ''], ['"yes"', 'yes'], ['2', 2], ['1.0', 1.0], ['[]', []]] as [$label, $bad]) {
        $r = wgct_create_request($json + ['ipv6' => $bad, 'unique' => $bad], false);
        wgct_check($t, "request: a malformed flag ({$label}) is refused on both fields, never defaulted",
    ...
    foreach ($cases as [$desc, $over, $field]) {
        $r = wgct_create_request(array_merge($json, $over), false);
        wgct_check($t, "request: {$desc} => {$field} error", array_keys($r['errors']) === [$field]);
    }

    wgct_check($t, 'redact: a secret in a message is replaced, short and empty secrets are ignored',
        wgct_redact("x {$priv} y ab", [$priv, 'ab', '']) === 'x [redacted] y ab');
    wgct_check($t, 'is_wg_key: base64 of 32 bytes only',
        wgct_is_wg_key($priv) && !wgct_is_wg_key('abc') && !wgct_is_wg_key(str_repeat('!', 44))
        && !wgct_is_wg_key(base64_encode(str_repeat('A', 31)) . 'AAAA'));

    return wgct_tally_report('wgconf', $t);
}
