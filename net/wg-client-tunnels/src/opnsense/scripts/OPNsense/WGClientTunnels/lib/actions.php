 ...
 * snapshot of core config -- wgct_action_snapshot() in writer.php, or the
...
const WGCT_INSTANCE_MAX = 251;
const WGCT_PORT_BASE = 51820;
...
const WGCT_GATEWAY_COPY_FIELDS = [
...
const WGCT_SENTINEL_PRIORITY = '254';
const WGCT_SENTINEL_IF_DESCR = 'NODEFAULT';
const WGCT_SENTINEL_LOOPBACK_DESCR = 'no-default sentinel';
const WGCT_SENTINEL_GW_DESCR = 'Sentinel: blocks non-native gateways from the default route (never carries traffic)';
 ...
 */
function wgct_ip_equal(string $a, string $b): bool {
 ...
 * @return bool
 */
function wgct_ip_in(string $ip, array $list): bool {
    foreach ($list as $candidate) {
        if (is_string($candidate) && wgct_ip_equal($ip, explode('/', $candidate)[0])) {
            return true;
        }
    }
    return false;
 ...
 */
function wgct_pick_instance(array $core, array $ports, bool $ipv6, string $ipv4Address): ?int {
    $usedN = [];
    $tunnelIps = [];
    ...
    $taken = fn (string $ip): bool => wgct_ip_in($ip, $gatewayIps) || wgct_ip_in($ip, $tunnelIps);
    for ($n = 1; $n <= WGCT_INSTANCE_MAX; $n++) {
        $far4 = '10.2.0.' . (3 + $n);
        if (isset($usedN[(string)$n]) || isset($usedPorts[(string)(WGCT_PORT_BASE + $n)])
            || $taken($far4) || wgct_ip_equal($far4, $ipv4Address)) {
            continue;
        }
 ...
 */
function wgct_pick_opt(array $interfaces): string {
    $i = 1;
 ...
 */
function wgct_interface_descr(string $name): string {
 ...
 */
function wgct_canonical_ip(string $ip): string {
 ...
 */
function wgct_instance_ipv6(array $core): array {
    $out = [];
    foreach ($core['instances'] as $inst) {
        foreach ($inst['tunneladdress'] as $address) {
 ...
 * @return bool
 */
function wgct_unique_convention(array $derived): bool {
    foreach ($derived['tunnels'] as $t) {
        if ($t['ipv6_address'] !== null && preg_match('/^wg(\d+)$/', $t['device'], $m) === 1
            && wgct_ip_equal(explode('/', $t['ipv6_address'])[0], "fd00::{$m[1]}:1")) {
            return true;
        }
    }
    return false;
 ...
 */
function wgct_is_nat_interface_key(string $key): bool {
 ...
 * @return string
 */
function wgct_retoken(string $text, array $map): string {
    if ($map === []) {
        return $text;
    }
 ...
 * @param array $req  wgct_create_request()['req'] with 'mtu' set
 * @param array $conf wgct_parse_wgquick()['public']
 * @return array see Task 3 Interfaces
 */
function wgct_plan_create(array $snap, array $req, array $conf): array {
    $core = $snap['core'];
    $e = [];
    $plan = [
    ...
    $descr = wgct_interface_descr($name);
    ...
    foreach ($core['gateways'] as $gwName => $g) {
        if ($g['monitor'] !== '' && wgct_ip_equal($g['monitor'], $monitor)) {
    ...
    if (wgct_ip_in($monitor, array_merge([$endpoint], array_column($core['peers'], 'serveraddress')))) {
    ...
    $instanceV6 = wgct_instance_ipv6($core);
    $sharedBy = null;
    ...
    if ($ipv6 && !$unique && $v6conf !== []) {
        if (count($v6conf) !== 1) {
            ...
            $v6addr = $v6conf[0];
            foreach ($core['instances'] as $inst) {
                if (wgct_ip_in(explode('/', $v6addr)[0], $inst['tunneladdress'])) {
    ...
    if ($req['template'] === '') {
        $known = array_merge(
            array_values(array_filter(array_map('strval', array_keys($core['interfaces'])), 'wgct_is_nat_interface_key')),
            $snap['aliases']
        );
    ...
    $n = wgct_pick_instance($core, $snap['ports'], $ipv6, explode('/', $v4)[0]);
    if ($n === null) {
        $e['general'] = sprintf('no free instance number from 1 to %d (its instance number, listen port, far gateway and fd00::N addresses must all be unused)', WGCT_INSTANCE_MAX);
    }
    if ($e !== [] || $n === null) {
        $plan['errors'] = $e;
        return $plan;
    ...
    /* the objects */
    $opt = wgct_pick_opt($core['interfaces']);
    $device = 'wg' . $n;
    $far4 = '10.2.0.' . (3 + $n);
    $next6 = "fd00::{$n}:2";
    ...
    $plan['ipv6'] = $ipv6;
    $plan['instance'] = [
        'enabled' => '1', 'name' => $name, 'instance' => (string)$n, 'port' => (string)(WGCT_PORT_BASE + $n),
    ...
    $plan['peer'] = [
        ...
        'serveraddress' => $endpoint, 'serverport' => $conf['endpoint_port'], 'keepalive' => WGCT_KEEPALIVE,
    ];
    $copy = function (?string $gwName) use ($snap): array {
        $fields = $gwName !== null ? ($snap['gateway_fields'][$gwName] ?? []) : [];
        return array_intersect_key($fields, array_flip(WGCT_GATEWAY_COPY_FIELDS));
    };
    ...
    if ($tpl !== null) {
        ...
        foreach ($tplRules as $r) {
            ...
            if (isset($fields['description'])) {
                $fields['description'] = wgct_retoken($fields['description'], $tokens);
            }
            $plan['nat'][] = $fields;
        }
    } else {
    ...
    $c = [sprintf(
        ...
        $name, $device, WGCT_PORT_BASE + $n, $plan['instance']['tunneladdress'], $far4, $req['mtu']
    )];
    if ($ipv6) {
        $c[] = $unique
 ...
 * @return array see Task 3 Interfaces
 */
function wgct_plan_rebind(array $snap, string $uuid, string $wan, string $staleUuid): array {
    $core = $snap['core'];
    ...
    $t = wgct_derive($core, [$uuid])['tunnels'][0];
 ...
 */
function wgct_plan_adopt(array $snap, string $uuid): array {
    $core = $snap['core'];
    ...
    if ($inst === null) {
        ...
        $plan['managed'][] = $uuid;
        $codes = array_column(wgct_derive($core, [$uuid])['tunnels'][0]['findings'], 'code');
 ...
 *
 * @param array  $snap the action snapshot
 * @param array  $refs wgct_refs_snapshot()
 * @param string $uuid the managed instance
 * @return array see Task 3 Interfaces
 */
function wgct_plan_remove(array $snap, array $refs, string $uuid): array {
    $core = $snap['core'];
    $plan = [
    ...
    $plan['errors'] = array_merge($plan['errors'], wgct_remove_refusals($refs, $plan['gateway_names'], $opt ?? '', $own));
 ...
 */
function wgct_plan_sentinel(array $interfaces, array $gateways, string $device, bool $loopbackAdded): array {
    ...
    if ($opt === null) {
        $opt = wgct_pick_opt($interfaces);
        $plan['assignment'] = 'add';
        $plan['changes'][] = "interface {$opt} (" . WGCT_SENTINEL_IF_DESCR . ") -> {$device}, dynamic gateway policy";
    } else {
    ...
    foreach ($gateways as $gwName => $g) {
        if (($g['priority'] ?? '') === WGCT_SENTINEL_PRIORITY && !in_array((string)$gwName, WGCT_SENTINELS, true)) {
            $plan['errors'][] = "gateway {$gwName} already uses priority " . WGCT_SENTINEL_PRIORITY . '; only the sentinels may';
        }
    }
    foreach (WGCT_SENTINELS as $family => $gwName) {
        $want = [
            'disabled' => '0', 'name' => $gwName, 'descr' => WGCT_SENTINEL_GW_DESCR, 'interface' => $opt, 'ipprotocol' => $family,
            'gateway' => '', 'defaultgw' => '0', 'monitor_disable' => '1', 'monitor_killstates' => '0', 'force_down' => '0',
            'priority' => WGCT_SENTINEL_PRIORITY,
        ];
        $have = $gateways[$gwName] ?? null;
        if ($have === null) {
            $plan['gateways'][$gwName] = ['action' => 'add', 'set' => $want];
            $plan['changes'][] = "gateway {$gwName} ({$family}, priority " . WGCT_SENTINEL_PRIORITY . ", no address) on {$opt}";
            continue;
        }
        $set = [];
...
function wgct_actions_fixture(): array {
    ...
    $thresholds = fn (array $o): array => array_merge(array_fill_keys(WGCT_GATEWAY_COPY_FIELDS, ''), $o);
    $snat = [
...
function wgct_actions_selftest(): int {
    ...
    wgct_check($t, 'create: template, config IPv6 shared with a managed tunnel => wg3 on opt4, unique addressing on',
    ...
    wgct_check($t, 'create: the peer allows both families, keepalive 25, no key material but the peer public key',
    ...
    wgct_check($t, 'create: gateways take the template thresholds and the spec behaviour fields',
    ...
    wgct_check($t, 'create: the change list names unique addressing and raises no NAT warning',
    ...
    $p = wgct_plan_create($s, $req, $conf);
    wgct_check($t, 'create: a copied rule\'s description names the new device and interface (whole tokens only)',
        $p['errors'] === [] && $p['nat'][1]['description'] === 'tun_a wg3 via opt4 (not wg10, opt11ip or xwg1)'
        && $p['nat'][0]['description'] === 'tailnet via tunnel');
    wgct_check($t, 'retoken: one pass, so a replacement is never replaced again; an empty map changes nothing',
        wgct_retoken('wg1 opt11 wg2', ['wg1' => 'wg2', 'wg2' => 'wg1']) === 'wg2 opt11 wg1' && wgct_retoken('wg1', []) === 'wg1');
    foreach ([
        ...
        $p = wgct_plan_create($s, $req, $conf);
        wgct_check($t, "create: a template NAT rule whose {$desc} => template refused, nothing planned",
    ...
    wgct_check($t, 'create: a rule that is not copied (disabled, or IPv6 with IPv6 off) may name the template interface',
        wgct_plan_create($s, ['ipv6' => false] + $req, $conf)['errors'] === []);
    ...
    $p = wgct_plan_create($snap, $req, $c);
    wgct_check($t, 'create: unshared config address, managed tunnels use fd00::N:1 => unique addressing on by default',
    ...
    $p = wgct_plan_create($noConvention, $req, $c);
    wgct_check($t, 'create: no convention in use, config address on no instance => off, the config address is used, next hop still fd00::3:2, reason text says why',
    ...
    $p = wgct_plan_create($s, $req, $c);
    wgct_check($t, 'create: config address already on an unmanaged instance (other spelling) => on by default',
        $p['errors'] === [] && $p['unique'] === true && $has($p['changes'], '2001:db8::2:2/128 is already on instance site_x'));
    $p = wgct_plan_create($snap, ['unique' => false] + $req, $c);
    wgct_check($t, 'create: convention in use but unique addressing explicitly off => allowed, the config address is used',
        $p['errors'] === [] && $p['unique'] === false && $p['instance']['tunneladdress'] === '10.2.0.2/32,2001:db8::2:2/128');

    $p = wgct_plan_create($snap, ['unique' => false] + $req, $conf);
    wgct_check($t, 'create: unique addressing off with an address already on an instance => refused', $err($p, 'unique'));

    /* a config without IPv6 (ruling 4) */
    $c = $conf;
    $c['addresses']['inet6'] = [];
    wgct_check($t, 'create: IPv6 requested but the config has no IPv6 address => refused on ipv6',
        $err(wgct_plan_create($snap, $req, $c), 'ipv6'));
    $p = wgct_plan_create($snap, ['ipv6' => null] + $req, $c);
    wgct_check($t, 'create: IPv6 unset and no IPv6 address in the config => IPv6 off, one gateway, IPv4-only peer',
        $p['errors'] === [] && $p['ipv6'] === false && count($p['gateways']) === 1 && $p['peer']['tunneladdress'] === '0.0.0.0/0'
        && $p['instance']['tunneladdress'] === '10.2.0.2/32');

    $p = wgct_plan_create($snap, ['ipv6' => false] + $req, $conf);
    wgct_check($t, 'create: IPv6 off => one gateway, IPv4-only peer and address, IPv4 NAT only',
        $p['errors'] === [] && count($p['gateways']) === 1 && $p['peer']['tunneladdress'] === '0.0.0.0/0'
        && $p['instance']['tunneladdress'] === '10.2.0.2/32' && count($p['nat']) === 2);

    $s = $snap;
    $s['ports']['i-x'] = '51823';
    wgct_check($t, 'create: listen port 51823 taken by another instance => N skips to 5', wgct_plan_create($s, $req, $conf)['n'] === 5);
    $s = $snap;
    $s['core']['gateways']['other'] = $gwRow('10.2.0.6');
    wgct_check($t, 'create: far gateway 10.2.0.6 taken by a gateway => N skips to 5', wgct_plan_create($s, $req, $conf)['n'] === 5);
    $s = $snap;
    $s['core']['instances']['i-x']['tunneladdress'] = ['10.2.0.6/32'];
    wgct_check($t, 'create: 10.2.0.6 is another instance\'s tunnel address => N skips to 5', wgct_plan_create($s, $req, $conf)['n'] === 5);
    $s = $snap;
    foreach (range(1, WGCT_INSTANCE_MAX) as $n) {
        $s['core']['instances']["f-{$n}"] = ['name' => "f{$n}", 'enabled' => false, 'instance' => (string)$n, 'mtu' => '', 'tunneladdress' => [], 'peers' => []];
    }
    wgct_check($t, 'create: every instance number 1..251 used => refused', $err(wgct_plan_create($s, $req, $conf), 'general'));
    ...
    foreach ([
        ...
        wgct_check($t, "create: {$desc} => {$field} refused", $err(wgct_plan_create($snap, $over + $req, $conf), $field));
    }

    $c = $conf;
    $c['endpoint_ip'] = '198.51.100.11';
    wgct_check($t, 'create: endpoint already routed via another WAN => wan refused', $err(wgct_plan_create($snap, $req, $c), 'wan'));
    $c['endpoint_ip'] = '198.51.100.13';
    $p = wgct_plan_create($snap, $req, $c);
    wgct_check($t, 'create: endpoint already routed via the same WAN => the route is kept, not duplicated',
        $p['errors'] === [] && $p['route'] === ['action' => 'keep', 'uuid' => 'r-d', 'fields' => []]);
    $s = $snap;
    $s['core']['routes']['r-z'] = ['network' => '198.51.100.12/32', 'gateway' => 'WAN_B', 'enabled' => false];
    $p = wgct_plan_create($s, $req, $conf);
    wgct_check($t, 'create: a disabled route to the endpoint => enabled and re-pointed',
    ...
    $p = wgct_plan_create($snap, $noTpl, $conf);
    wgct_check($t, 'create: no template => one NAT rule per source and family, destination any, target the interface address',
    ...
    wgct_check($t, 'create: no template => gateways get the spec behaviour fields and core\'s default thresholds',
        $p['gateways'][0]['fields']['fargw'] === '1' && $p['gateways'][0]['fields']['monitor_disable'] === '0'
        && !isset($p['gateways'][0]['fields']['latencylow']));
    $p = wgct_plan_create($snap, ['nat' => ['inet' => [], 'inet6' => []]] + $noTpl, $conf);
    wgct_check($t, 'create: no template, no NAT sources => allowed, with nat-missing warnings for both families',
        $p['errors'] === [] && $has($p['changes'], 'WARNING nat-missing: no IPv4') && $has($p['changes'], 'WARNING nat-missing: no IPv6'));
    wgct_check($t, 'create: an unknown NAT source => nat4 refused',
        $err(wgct_plan_create($snap, ['nat' => ['inet' => ['bogus'], 'inet6' => []]] + $noTpl, $conf), 'nat4'));
    wgct_check($t, 'create: a virtual interface key (wireguard group) is not a NAT source => nat4 refused',
        $err(wgct_plan_create($snap, ['nat' => ['inet' => ['wireguard'], 'inet6' => []]] + $noTpl, $conf), 'nat4'));
    $p = wgct_plan_create($snap, ['ipv6' => false] + $noTpl, $conf);
    wgct_check($t, 'create: IPv6 off ignores the IPv6 NAT sources', $p['errors'] === [] && count($p['nat']) === 2);
    $s = $snap;
    ...
    $p = wgct_plan_rebind($moved, 'i-a', 'WAN_B', 'r-a');
    wgct_check($t, 'rebind: unbound tunnel => route to the new endpoint, the stale route deleted with its kernel route',
    ...
    wgct_check($t, 'rebind: no stale route chosen => nothing deleted', wgct_plan_rebind($moved, 'i-a', 'WAN_B', '')['delete'] === null);
    wgct_check($t, 'rebind: a live binding route is not a stale candidate => refused', wgct_plan_rebind($moved, 'i-a', 'WAN_B', 'r-b')['errors'] !== []);
    wgct_check($t, 'rebind: a bound tunnel => refused', wgct_plan_rebind($snap, 'i-a', 'WAN_B', '')['errors'] !== []);
    $s = $moved;
    $s['core']['routes']['r-z'] = ['network' => '198.51.100.20/32', 'gateway' => 'WAN_A', 'enabled' => false];
    wgct_check($t, 'rebind: a disabled route to the endpoint => updated in place',
        wgct_plan_rebind($s, 'i-a', 'WAN_B', '')['route'] === ['action' => 'update', 'uuid' => 'r-z', 'fields' => ['gateway' => 'WAN_B', 'enabled' => '1']]);
    wgct_check($t, 'rebind: WAN on a WireGuard interface => refused', wgct_plan_rebind($moved, 'i-a', 'tun_b', '')['errors'] !== []);
    wgct_check($t, 'rebind: an unmanaged instance => refused', wgct_plan_rebind($moved, 'i-x', 'WAN_B', '')['errors'] !== []);
    $s = $snap;
    ...
    $stale = array_values(array_filter(
        wgct_derive($s['core'], ['i-a'])['tunnels'][0]['findings'],
        fn (array $f): bool => $f['code'] === 'stale-route'
    ));
    wgct_check($t, 'binding: two routes to one stale IP => one stale-route entry, the last route winning (text as before the refactor)',
        count($stale) === 1 && $stale[0]['detail'] === '198.51.100.10/32 via WAN_B');

    /* ---- adopt ---- */
    $p = wgct_plan_adopt($snap, 'i-x');
    wgct_check($t, 'adopt: an unmanaged instance joins the managed list; its findings are previewed',
        $p['errors'] === [] && $p['managed'] === ['i-a', 'i-b', 'i-d', 'i-x'] && $p['changes'][0] === 'manage site_x (wg7)'
        && $has($p['changes'], 'findings once managed'));
    wgct_check($t, 'adopt: already managed => refused', wgct_plan_adopt($snap, 'i-a')['errors'] !== []);
    wgct_check($t, 'adopt: no such instance => refused', wgct_plan_adopt($snap, 'i-nope')['errors'] !== []);

    /* ---- remove ---- */
    $group = [['id' => 'group:grp_a', 'what' => 'gateway group grp_a', 'gateways' => ['tun_a', 'tun_b'], 'interfaces' => []]];
    wgct_check($t, 'remove: a gateway group lists it => refused, and the refusal says why',
        wgct_plan_remove($snap, $group, 'i-a')['errors'] === ['gateway group grp_a uses gateway tun_a']);
    $p = wgct_plan_remove($snap, [], 'i-d');
    wgct_check($t, 'remove: an unreferenced tunnel => peer, instance, interface, its gateways, endpoint route, NAT, managed and held entries',
    ...
    $p = wgct_plan_remove($s, [], 'i-d');
    wgct_check($t, 'remove: another instance\'s peer uses the same endpoint => the route is kept',
        $p['errors'] === [] && $p['routes'] === [] && $has($p['changes'], 'KEEP static route 198.51.100.13/32'));
    $s = $snap;
    $s['core']['instances']['i-x']['peers'] = ['p-x', 'p-d'];
    wgct_check($t, 'remove: its peer is also another instance\'s peer => refused',
        in_array('peer tun_d is also a peer of instance site_x', wgct_plan_remove($s, [], 'i-d')['errors'], true));
    $s = $snap;
    $s['managed'][] = 'i-gone';
    $p = wgct_plan_remove($s, [], 'i-gone');
    wgct_check($t, 'remove: instance already deleted => only the managed entry goes',
        $p['errors'] === [] && $p['instance'] === null && $p['managed'] === ['i-a', 'i-b', 'i-d']);
    wgct_check($t, 'remove: an unmanaged instance => refused', wgct_plan_remove($snap, [], 'i-x')['errors'] !== []);
    $ownRefs = [
    ...
    $want = fn (string $family, string $name, string $opt): array => [
        'disabled' => '0', 'name' => $name, 'descr' => WGCT_SENTINEL_GW_DESCR, 'interface' => $opt, 'ipprotocol' => $family,
    ...
    $p = wgct_plan_sentinel($ifs, $gws, 'lo1', true);
    wgct_check($t, 'sentinel: blank slate => lo1 assigned on opt2 with dynamic gateway policy, both sentinels added',
    ...
    wgct_check($t, 'sentinel: blank slate => loopback, interface and routing applies, in that order',
    ...
    $p = wgct_plan_sentinel($ifs2, $gws2, 'lo1', false);
    wgct_check($t, 'sentinel: already right => nothing to change, nothing to apply',
    ...
    $p = wgct_plan_sentinel($x, $gws2, 'lo1', false);
    wgct_check($t, 'sentinel: dynamic gateway policy missing => set', $p['assignment'] === 'policy');
    wgct_check($t, 'sentinel: policy only => the interface apply alone', $p['steps'] === [['interface reconfigure', ['opt10']]]);
    $x = $ifs2;
    $x['opt10']['ipaddr'] = '192.0.2.9';
    wgct_check($t, 'sentinel: the loopback has an address => refused', wgct_plan_sentinel($x, $gws2, 'lo1', false)['errors'] !== []);
    $y = $gws2;
    $y['WAN_A']['priority'] = '254';
    wgct_check($t, 'sentinel: another gateway uses priority 254 => refused',
        in_array('gateway WAN_A already uses priority 254; only the sentinels may', wgct_plan_sentinel($ifs2, $y, 'lo1', false)['errors'], true));
    $y = $gws2;
    $y['NO_DEFAULT6']['descr'] = 'old';
    $p = wgct_plan_sentinel($ifs2, $y, 'lo1', false);
    wgct_check($t, 'sentinel: a drifted field => updated alone',
        $p['gateways']['NO_DEFAULT6'] === ['action' => 'update', 'set' => ['descr' => WGCT_SENTINEL_GW_DESCR]]);
    wgct_check($t, 'sentinel: a description-only drift => saved with no apply at all (never an interface reconfigure)', $p['steps'] === []);
    $y = $gws2;
    $y['NO_DEFAULT4']['monitor_disable'] = '0';
    wgct_check($t, 'sentinel: a routing field drifted => the routing apply alone',
        wgct_plan_sentinel($ifs2, $y, 'lo1', false)['steps'] === [['interface routes configure', []]]);

    return wgct_tally_report('actions', $t);
}
