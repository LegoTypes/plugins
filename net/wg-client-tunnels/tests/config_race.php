 ...
 *
 * Race test for wgct_locked_commit(). Run on the firewall as root, with the
...
require_once '/usr/local/opnsense/scripts/OPNsense/WGClientTunnels/lib/mirror.php';
...
function race_set_locked($idx, $text) {
    wgct_locked_commit(function () use ($idx, $text) {
        $mdl = new Gateways();
...
try {
    ...
    /* Fix: the same interleaving through wgct_locked_commit keeps both.
