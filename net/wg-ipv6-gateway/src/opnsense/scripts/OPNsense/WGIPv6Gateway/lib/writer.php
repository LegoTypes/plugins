 ...
 * wgipv6_routing_action() (apply.php) or, for the API's Create, for the
 ...
 */
function wgipv6_reset_field_caches(): void {
 ...
 */
function wgipv6_model_errors(\OPNsense\Base\BaseModel $mdl, string $label): array {
    $out = [];
 ...
 */
function wgipv6_action_snapshot(): array {
    ...
    foreach ((new \OPNsense\Routing\Gateways())->gateway_item->iterateItems() as $gw) {
        $row = [];
        foreach (WGIPV6_GATEWAY_COPY_FIELDS as $field) {
    ...
    foreach ((new \OPNsense\Firewall\Filter())->snatrules->rule->iterateItems() as $uuid => $rule) {
        ...
        foreach ($rule->iterateItems() as $field => $node) {
            if (!in_array((string)$field, WGIPV6_SNAT_SKIP_FIELDS, true) && !$node->getInternalIsVolatile()) {
    ...
    $groups = [];
    foreach (wgipv6_xml_list($root, 'ifgroups', 'ifgroupentry') as $group) {
        $groups[] = (string)$group->ifname;
    }
    return [
        'core' => wgipv6_core_snapshot(),
        'managed' => wgipv6_split_csv((string)$mdl->managed),
        'held' => wgipv6_split_csv((string)$mdl->held),
        'ports' => $ports,
...
/**
 * wgipv6_locked_commit() for an action. $mutate returns ['save' => bool,
 * 'result' => wgipv6_result()]. Unless it saved, config is re-read.
 *
 ...
 */
function wgipv6_action_commit(callable $mutate, string $description): array {
    try {
        $commit = wgipv6_locked_commit($mutate, 'wg client tunnels: ' . $description);
    } catch (\Throwable $e) {
 ...
 *
 * @param array $raw         the API form or the CLI JSON (see wgipv6_create_request())
 ...
 */
function wgipv6_create_prepare(#[\SensitiveParameter] array $raw, bool $mtuRequired): array {
    $in = wgipv6_create_request($raw, $mtuRequired);
    $prep = ['errors' => $in['errors'], 'req' => $in['req'], 'public' => [], 'secret' => ['privkey' => '', 'psk' => '', 'pubkey' => ''], 'notes' => []];
    if ($in['text'] !== '') {
        $conf = wgipv6_parse_wgquick($in['text']);
        if ($conf['errors'] !== []) {
    ...
    $pubkey = wgipv6_derive_pubkey($prep['secret']['privkey']);
    if ($pubkey === null) {
    ...
    if ($prep['req']['mtu'] === null) {
        $m = wgipv6_measure_mtu($prep['req']['wan'], $prep['public']['endpoint_ip']);
        if (!$m['ok']) {
 ...
 *
 * @param array $plan   wgipv6_plan_create() without errors
 * @param array $secret ['privkey', 'psk', 'pubkey']
 * @return array{errors: list<string>, uuid: string}
 */
function wgipv6_write_create(array $plan, #[\SensitiveParameter] array $secret): array {
    $root = Config::getInstance()->object();
    ...
    $errors = wgipv6_model_errors($client, 'wireguard peer');
    if ($errors !== []) {
    ...
    $errors = wgipv6_model_errors($server, 'wireguard instance');
    if ($errors !== []) {
        return ['errors' => $errors, 'uuid' => ''];
    }
    ...
    $errors = wgipv6_model_errors($gateways, 'gateways');
    if ($errors !== []) {
        return ['errors' => $errors, 'uuid' => ''];
    }
    ...
    $errors = wgipv6_model_errors($routes, 'static routes');
    if ($errors !== []) {
        return ['errors' => $errors, 'uuid' => ''];
    }
    ...
    $errors = wgipv6_model_errors($filter, 'outbound NAT');
    if ($errors !== []) {
        return ['errors' => $errors, 'uuid' => ''];
    }
    ...
    $errors = wgipv6_model_errors($mdl, 'plugin');
    if ($errors !== []) {
        return ['errors' => $errors, 'uuid' => ''];
    }
...
function wgipv6_create_commit(#[\SensitiveParameter] array $prep, bool $dry): array {
    $result = wgipv6_action_commit(function () use ($prep, $dry): array {
        $plan = wgipv6_plan_create(wgipv6_action_snapshot(), $prep['req'], $prep['public']);
        $changes = array_merge($prep['notes'], $plan['changes']);
        if ($plan['errors'] !== []) {
            return ['save' => false, 'result' => wgipv6_result(['errors' => $plan['errors'], 'changes' => $changes, 'dry' => $dry])];
        }
        $written = wgipv6_write_create($plan, $prep['secret']);
        if ($written['errors'] !== []) {
            return ['save' => false, 'result' => wgipv6_result(['errors' => ['general' => implode('; ', $written['errors'])], 'changes' => $changes, 'dry' => $dry])];
        }
    ...
    if ($result['saved']) {
        wgipv6_set_apply_pending($result['uuid'], true);
    }
    return $result;
}
 ...
 * @return array wgipv6_result()
 */
function wgipv6_rebind_commit(string $uuid, string $wan, string $staleUuid, bool $dry): array {
    return wgipv6_action_commit(function () use ($uuid, $wan, $staleUuid, $dry): array {
        $plan = wgipv6_plan_rebind(wgipv6_action_snapshot(), $uuid, $wan, $staleUuid);
        if ($plan['errors'] !== []) {
            return ['save' => false, 'result' => wgipv6_result(['errors' => $plan['errors'], 'changes' => $plan['changes'], 'dry' => $dry, 'uuid' => $uuid])];
        }
        $routes = new \OPNsense\Routes\Route();
        if ($plan['route']['action'] === 'add') {
            $routes->route->Add()->setNodes($plan['route']['fields']);
            ...
            $node = $routes->getNodeByReference('route.' . $plan['route']['uuid']);
            if ($node === null) {
                return ['save' => false, 'result' => wgipv6_result(['errors' => ['the route to update vanished'], 'dry' => $dry, 'uuid' => $uuid])];
            }
            $node->setNodes($plan['route']['fields']);
        }
        if ($plan['delete'] !== null) {
        ...
        $errors = wgipv6_model_errors($routes, 'static routes');
        if ($errors !== []) {
            return ['save' => false, 'result' => wgipv6_result(['errors' => $errors, 'changes' => $plan['changes'], 'dry' => $dry, 'uuid' => $uuid])];
        }
        if (!$dry) {
            $routes->serializeToConfig();
        }
 ...
 * @return array wgipv6_result()
 */
function wgipv6_adopt_commit(string $uuid, bool $dry): array {
    return wgipv6_action_commit(function () use ($uuid, $dry): array {
        $plan = wgipv6_plan_adopt(wgipv6_action_snapshot(), $uuid);
        if ($plan['errors'] !== []) {
            return ['save' => false, 'result' => wgipv6_result(['errors' => $plan['errors'], 'dry' => $dry, 'uuid' => $uuid])];
        }
        $mdl = new \OPNsense\WGIPv6Gateway\WGIPv6Gateway();
        $mdl->managed = implode(',', $plan['managed']);
        $errors = wgipv6_model_errors($mdl, 'plugin');
        if ($errors !== []) {
            return ['save' => false, 'result' => wgipv6_result(['errors' => $errors, 'dry' => $dry, 'uuid' => $uuid])];
        }
        if (!$dry) {
            $mdl->serializeToConfig();
        }
 ...
 * @return array wgipv6_result()
 */
function wgipv6_remove_commit(string $uuid, bool $dry): array {
    return wgipv6_action_commit(function () use ($uuid, $dry): array {
        $snap = wgipv6_action_snapshot();
        $plan = wgipv6_plan_remove($snap, wgipv6_refs_snapshot($snap['core']['groups']), $uuid);
        if ($plan['errors'] !== []) {
            return ['save' => false, 'result' => wgipv6_result(['errors' => $plan['errors'], 'changes' => $plan['changes'], 'dry' => $dry, 'uuid' => $uuid])];
        }
        $client = new \OPNsense\Wireguard\Client();
        ...
        foreach ($models as $label => $model) {
            $errors = array_merge($errors, wgipv6_model_errors($model, $label));
        }
        if ($errors !== []) {
            return ['save' => false, 'result' => wgipv6_result(['errors' => $errors, 'changes' => $plan['changes'], 'dry' => $dry, 'uuid' => $uuid])];
        }
 ...
 * @return array wgipv6_result()
 */
function wgipv6_sentinel_commit(bool $dry): array {
    return wgipv6_action_commit(function () use ($dry): array {
        $root = Config::getInstance()->object();
        $changes = [];
        $loopbacks = new \OPNsense\Interfaces\Loopback();
        $loopback = null;
        foreach ($loopbacks->loopback->iterateItems() as $node) {
            if ((string)$node->description === WGIPV6_SENTINEL_LOOPBACK_DESCR) {
                $loopback = $node;
            }
        }
        ...
        if ($loopback === null) {
            ...
            $loopback->description = WGIPV6_SENTINEL_LOOPBACK_DESCR;
            $changes[] = 'loopback device (' . WGIPV6_SENTINEL_LOOPBACK_DESCR . ')';
        }
        $errors = wgipv6_model_errors($loopbacks, 'loopback');
        ...
        $plan = wgipv6_plan_sentinel($interfaces, $gateways, $device, $loopbackAdded);
        $changes = array_merge($changes, $plan['changes']);
        $errors = array_merge($errors, $plan['errors']);
        if ($errors !== []) {
            return ['save' => false, 'result' => wgipv6_result(['errors' => $errors, 'changes' => $changes, 'dry' => $dry])];
        }
        if ($changes === []) {
            return ['save' => false, 'result' => wgipv6_result(['ok' => true, 'dry' => $dry])];
        }
        $opt = $plan['opt'];
        if ($plan['assignment'] === 'add') {
            $node = $root->interfaces->addChild($opt);
            $node->addChild('if', $device);
            $node->addChild('descr', WGIPV6_SENTINEL_IF_DESCR);
        ...
        $errors = wgipv6_model_errors($model, 'gateways');
        if ($errors !== []) {
            return ['save' => false, 'result' => wgipv6_result(['errors' => $errors, 'changes' => $changes, 'dry' => $dry])];
        }
        $model->serializeToConfig();
        ...
        return ['save' => !$dry, 'result' => wgipv6_result([
            ...
            'gateways' => $routing ? array_values(WGIPV6_SENTINELS) : [], 'steps' => $plan['steps'],
        ])];
    }, 'ensure sentinel');
}
